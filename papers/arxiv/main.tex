...
\section{Results}
As seen in Figure~\ref{fig:attendance}, to contextualize the physical scale of the event before analyzing its digital textual patterns, a quantitative dataset of official on-site attendance metrics was compiled across the 2015--2026 window. Data collection proceeded across three sequential stages:
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/covid_yoga_trends.png}
...
Aligned with H1, our empirical modeling provides robust evidence demonstrating that the identified discourse clusters are structurally partitioned, exhibiting highly significant differences in the Discourse Structure Index (DSI). The distribution of DSI values across the independent discourse communities is visualised in the boxplot distribution of Figure~\ref{fig:boxplot}, illustrating substantial rhetorical stratification across institutional environments. 

The omnibus effect of cluster membership on DSI scores is confirmed by a one-way Analysis of Variance (ANOVA), which indicates a highly significant division of communication types across the IDY ecosystem:
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Cluster-year structure density heatmap tracking longitudinal distribution intensity.}
    \label{fig:heatmap}
\end{figure}

\subsection{Mixed-Effects Model}
As detailed in Table~\ref{tab:model_comparison}, mixed-effects models outperform the aggregate OLS specification, indicating strong channel-level variance. 
...
The predicted trajectories from the mixed-effects model are presented in Figure~\ref{fig:mixed_effects}. This figure illustrates the relative evolution of the model terms and further demonstrates the descriptive superiority of the mixed-effects specification over the corresponding OLS baseline model.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/mixed-effects.png}
    \caption{Mixed-effects model predicted DSI trajectories across independent cluster classifications.}
    \label{fig:mixed_effects}
\end{figure}

This progressive macro-level paradigm shift and structural alignment across institutional categories is empirically map-indexed across the broader bibliometric field in Figure~\ref{fig:pubmed_macro}. It is clear the trend toward yogic informed sustainable governance has increased over time. While the increase in neuro-immodulation from 2019 onwards has also outpaced the more stable pulmonary therapeutic focus.
...
\begin{figure}[htbp]
    ...
    \label{fig:niche_trajectory}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/yoga_proximity_chart.png}
