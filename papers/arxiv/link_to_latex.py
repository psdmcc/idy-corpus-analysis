...
def link_project_to_latex():
    # =========================================================================
    # DYNAMIC AUTO-PATH DETECTION (No manual path entries required)
    # =========================================================================
    ...
    print("Initializing Smart Image Linker & Injector...")
    ...
    for img in images:
        ...
        # Check current folder, parent folder, or grandparent folder (IDY_Project)
        ...
        elif os.path.exists(os.path.join(grandparent_dir, img)):
            source_path = os.path.join(grandparent_dir, img)
        ...
        if source_path:
            ...
            print(f"⚠️ Warning: '{img}' not found in current or parent folders. Re-run your plot scripts first.")
    ...
    # Using a Python raw string prefix (r"...") to completely neutralize backslash warnings
    latex_code_to_inject = r"""
    ...
    try:
        ...
        if "idy_separated_raw_trends_600dpi.png" in content:
            ...
            if r"\end{document}" in content:
                content = content.replace(r"\end{document}", latex_code_to_inject + "\n\\end{document}")
